...
class Private_VC_Commands(cmds.Cog , name = "private vc" , description = "Commands that can only be used while being in your private voice channel") :
    ...
    async def check_vc(ctx) :
        return await TheBot.check_vc(ctx)


    @cmds.slash_command(name = "give" , description = "Transfers your private voice channel to the given member.")
    @cmds.cooldown(rate = 3 , per = 1)
    @cmds.check(check_vc)
    async def give(self , ctx , member : discord.Option(discord.Member , "Enter the member to give the voice channel to.") = None) :
        if not member :
    ...
    @cmds.slash_command(name = "max" , description = "Limits your private voice channel to the given amount of members.")
    @cmds.cooldown(rate = 3 , per = 1)
    @cmds.check(check_vc)
    async def max_cmd(self , ctx , amount : discord.Option(int , "Enter the amount of users to limit your voice channel to.") = 0) :
        if amount < 0 :
            return await self.bot.send(ctx , "Provide a valide amount!" , 0xff7f00)
        await ctx.author.voice.channel.edit(user_limit = amount)


    @cmds.slash_command(name = "lockvc" , description = "Locks your private voice channel from the given member.") # Locks the private voice channel
    @cmds.cooldown(rate = 3 , per = 1)
    @cmds.check(check_vc)
    async def lockvc(self , ctx , member : discord.Option(discord.Member , "Enter the member to lock your voice channel from.")) :
    ...
    @cmds.slash_command(name = "unlockvc" , description = "Unlocks your private voice channel from the given member.") # Unlocks the private voice channel
    @cmds.cooldown(rate = 3 , per = 1)
    @cmds.check(check_vc)
    async def unlockvc(self , ctx , member : discord.Option(discord.Member , "Enter the member to unlock your voice channel from.")) :
        if member == ctx.author :
    ...
    @cmds.slash_command(name = "renamevc" , description = "Renames your private voice channel to the given name.") # Renames the private voice channel
    @cmds.cooldown(rate = 3 , per = 1)
    @cmds.check(check_vc)
    async def renamevc(self , ctx , name : discord.Option(str , "Enter thee name you want to give to your channel.")) :
        if len(name) > 99 :
