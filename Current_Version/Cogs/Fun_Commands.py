...
class Fun_Commands(cmds.Cog , name = "fun" , description = "Commands that don't do something useful, used for fun.") :
    ...
    async def kill(self , ctx , dead : discord.Option(discord.Member , "Enter the member to kill.")) :
        killer = ctx.author
    ...
    @cmds.cooldown(rate = 3 , per = 1)
    async def revive(self , ctx , * , alive : discord.Option(discord.Member , "Enter the member to revive.")) :
        healer = ctx.author
