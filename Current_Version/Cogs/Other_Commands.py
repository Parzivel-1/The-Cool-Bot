...
class Other_Commands(cmds.Cog , name = "other" , description = "Other commands.") :
    ...
    @cmds.command(name = "test" , aliases = ("Test" , "TEST" , "t" , "T") , description = "`test` Test!") # Test command
    @cmds.cooldown(rate = 3 , per = 1)
    async def test(self , ctx) :
        await self.bot.send(ctx , "Success!" , 0x00ff00)


    @cmds.command(name = "ping" , aliases = ("Ping" , "PING" , "p" , "P") , description = "`ping` Checks the latency of the bot.") # Sends the ping
    @cmds.cooldown(rate = 3 , per = 1)
    ...
    @cmds.command(name = "info" , aliases = ("Info" , "INFO" , "i" , "I" , "user" , "User" , "USER" , "u" , "U") , description = "`info <member>` Shows information about the given member") # Sends member info
    @cmds.cooldown(rate = 3 , per = 1)
    async def info(self , ctx , * , member : discord.Member = None) :
        ...
        if not roles : text = f"{member.display_name} doesn't have any roles."
        else : text = "\n".join(role.mention for role in roles)
        msg = discord.Embed(title = "Member Information:" , color = 0x00ff00)
        if member == owner : msg.add_field(name = "The Owner:" , value = f"{member.display_name} is the owner of the server!" , inline = False)
    ...
    @cmds.command(name = "server" , aliases = ("Server" , "SERVER" , "s" , "S") , description = "`server` Shows information about the server.") # Sends server info
    @cmds.cooldown(rate = 3 , per = 1)
    async def server(self , ctx) :
        ...
        msg.set_thumbnail(url = ctx.guild.banner_url)
        msg.set_image(url = ctx.guild.icon_url)
        msg.set_author(name = f"{ctx.author} || {ctx.author.nick}" if ctx.author.nick else ctx.author , icon_url = ctx.author.avatar)
        await ctx.reply(embed = msg)



def setup(bot) : bot.add_cog(Other_Commands(bot))